/**
 * Created by Web on 2017/3/15.
 */
    $('#site_header').load('data/header.php', function () {
        if(sessionStorage['name']){
            $('#top_total').html("欢迎回来： "+""+"<a href='#'>"+sessionStorage['name']+"</a>");
        }
    });
...
$.ajax({
    ...
    success: function(data){
        ...
        var html ="<li><a href='#'> <img src='img/shouye/hr_0.jpg'/> <div> <p>4GB+64GB/双卡全网通、5000mAh电池</p> <span>￥<b>2699</b>起</span> </div></a></li>";
        for(var p of data){
            //console.log(p[0]);
            html+="<li class='hot_detail'><a href='#'> <img src='"+p[1]+"'> <div> <span>"+p[2]+"</span><br/> <a href='#'>"+p[3]+"</a> <p>￥<b>"+p[4]+"</b></p> </div> </a></li>";
        }
        $('#main .hot ul').html(html);
    },
    error: function(){
...
$.ajax({
    ...
    success: function(data){
        ...
        for(var p of data){
            //console.log(p[0]);
            html+="<li><a href='#'> <img src='"+p[1]+"'/> <ul> <li class='jingpin_first'><a href='#'>"+p[2]+"</a></li> <li><a href='#'>"+p[3]+"</a></li> <li class='jingpin_last'><a href='#'>￥<span>"+p[4]+"</span></a></li> </ul> </a></li>";
        }
        $('#main .jingpin>ul').html(html);
    },
    error: function(){
        alert('异步请求商品列表出错！请检查响应消息');
    }
